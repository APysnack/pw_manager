...
public class MngUserPanel extends JPanel
    ...
    public void updateLayout(String layoutName) {
        ...
        if (Objects.equals(layoutName, "main")) {
        ...
        if (source == userComboBox) {
            ...
            if (user.getEditPermission()) {
                ...
                if (response == 0) {
                    ...
                    if((numAdmins < 2)
                            && (moddedUser.getAddPermission() && moddedUser.getEditPermission()
                            && moddedUser.getDeletePermission())
                            && (!canAdd || !canEdit || canDelete)) {
                        JOptionPane.showMessageDialog(null,
                                "User not modified. There must be at least one user with all permissions (add/edit/delete)");

                    } else {
                        if ((Arrays.equals(password, passwordConfirm)) || !passwordIsBeingChanged) {
